...
const OSManager: React.FC = () => {
    ...
    return (
        <div className="os-manager-container" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            <header style={{ padding: '1.5rem 2rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h1 style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>Ordens de Serviço</h1>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Gerenciamento de campo e agendamento técnico.</p>
                </div>
                <div style={{ display: 'flex', gap: '10px', background: 'var(--bg-surface)', padding: '4px', borderRadius: ' var(--radius-md)', border: '1px solid var(--border)' }}>
                    <button onClick={() => setView('list')} style={{ padding: '6px 16px', borderRadius: 'var(--radius-sm)', border: 'none', background: view === 'list' ? 'var(--accent)' : 'transparent', color: view === 'list' ? '#fff' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: 600 }}>Lista</button>
                    <button onClick={() => setView('map')} style={{ padding: '6px 16px', borderRadius: 'var(--radius-sm)', border: 'none', background: view === 'map' ? 'var(--accent)' : 'transparent', color: view === 'map' ? '#fff' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: 600 }}>Mapa</button>
                </div>
            </header>
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                {view === 'list' ? (
                    <div className="ic-sidebar-scroll" style={{ flex: 1, padding: '2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px', alignContent: 'start' }}>
                        {oss.map(os => (
                            <div key={os.id} className="os-card" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '12px', transition: 'transform 0.2s', cursor: 'pointer' }}>
                                ...
                                <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <button style={{ padding: '6px 12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--accent)', background: 'transparent', color: 'var(--accent)', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>
                                        Gerenciar
                                    </button>
                                </div>
                            </div>
                        ...
                        <MapContainer center={[-23.5505, -46.6333]} zoom={13} style={{ height: '100%', width: '100%' }}>
                            ...
                            {oss.map(os => (
                                <Marker key={os.id} position={[os.assinante.enderecos[0].latitude, os.assinante.enderecos[0].longitude]}>
                                    <Popup>
                                        <div style={{ color: '#000' }}>
                                            <strong>{os.tipo}</strong><br />
                                            Cliente: {os.assinante.nome}<br />
                                            Status: {os.status}
                                        </div>
                                    </Popup>
                                </Marker>
